...
import { State, MatchLabel, StartMessage, UpdateMessage, DoneMessage, MoveMessage, RpcFindMatchRequest, RpcFindMatchResponse } from './interfaces';
import { Board, gameServerConfig, Mark, OpCode, winningPositions } from '../utils/constants';
import { msecToSec } from '../utils/utilities';
...
let matchInit: nkruntime.MatchInitFunction<State> = function (ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, params: {[key: string]: string}) {
    ...
    logger.debug('Match init: tickRate: %d, fast: %t, ai: %t', gameServerConfig.tickRate, fast, ai);

    return {
        state,
        tickRate: gameServerConfig.tickRate,
    ...
    for (const presence of presences) {
        ...
        if (state.playing) {
            ...
            let update: UpdateMessage = {
                board: state.board,
                mark: state.mark,
                deadline: t + Math.floor(state.deadlineRemainingTicks/gameServerConfig.tickRate),
            }
            ...
            let done: DoneMessage = {
                ...
                nextGameStart: t + Math.floor(state.nextGameRemainingTicks/gameServerConfig.tickRate)
            }
            // Send a message to the user that just joined.
...
let matchLoop: nkruntime.MatchLoopFunction<State> = function(ctx: nkruntime.Context, logger: nkruntime.Logger, nk: nkruntime.Nakama, dispatcher: nkruntime.MatchDispatcher, tick: number, state: State, messages: nkruntime.MatchMessage[]) {
    ...
    if (connectedPlayers(state) + state.joinsInProgress === 0) {
        state.emptyTicks++;
        if (state.emptyTicks >= gameServerConfig.maxEmptySec * gameServerConfig.tickRate) {
        ...
        switch (message.opCode) {
            case OpCode.MOVE:
                ...
                if (winner) {
                    ...
                    state.nextGameRemainingTicks = gameServerConfig.delaybetweenGamesSec * gameServerConfig.tickRate;
                }
...
function calculateDeadlineTicks(l: MatchLabel): number {
    if (l.fast === 1) {
        return gameServerConfig.turnTimeFastSec * gameServerConfig.tickRate;
    } else {
        return gameServerConfig.turnTimeNormalSec * gameServerConfig.tickRate;
    }
}
